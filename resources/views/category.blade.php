      ...
      <div class="row mt-3">
        <div class="col-12 col-md-4 mb-2 px-2 px-lg-3 d-lg-block d-none">
          ...
          <div class="catalog__ategory col-12 bg-white px-0 custom-lined-category">
            @if(!$name->parent)
              <h4 class="py-2 px-3 category-Border-gray"><a class="text-center text-decoration-none subcategory text-secondary py-4" href="{{ route('ft-category.category', $name->slug) }}">@if($name->icon)<img src="/storage/{{ $name->icon }}" height="28" width="28" alt="" class="mr-2">@endif{{ $name->name }}</a></h4>
            @elseif(!$name->parent->parent)
              @if(isset($name->childrens->first()->id))
                <h4 class="py-2 px-3 category-Border-gray"><a class="text-center text-decoration-none subcategory text-secondary py-4" href="{{ route('ft-category.category', $name->slug) }}">@if($name->icon)<img src="/storage/{{ $name->icon }}" height="28" width="28" alt="" class="mr-2">@endif{{ $name->name }}</a></h4>
              @endif
            ...
            @elseif(!$name->parent->parent->parent)
              <h4 class="py-2 px-3 category-Border-gray"><a class="text-center text-decoration-none subcategory text-secondary py-4" href="{{ route('ft-category.category', $name->slug) }}">@if($name->icon)<img src="/storage/{{ $name->icon }}" height="28" width="28" alt="" class="mr-2">@endif{{ $name->name }}</a></h4>
            @else
              <h4 class="py-2 px-3 category-Border-gray"><a class="text-center text-decoration-none subcategory text-secondary py-4" href="{{ route('ft-category.category', $name->parent ? $name->parent->slug : $name->slug) }}">@if($name->parent->icon)<img src="/storage/{{ $name->parent->icon }}" height="28" width="28" alt="" class="mr-2">@endif{{ $name->parent->name }}</a></h4>
            @endif
