    ...
    <div class="container">
      ...
      <div class="row d-flex justify-content-between" >
        ...
        <div class="col-lg-5 col-12 w-100 add-product" >
          ...
          <h5 class="text-secondary mb-4 d-flex d-lg-none" >Изменить Товар</h5>
          </div>
                ...
                @foreach(json_decode($product->gallery) as $gallery)
                    <div class="preview-image col-3 col-lg-4 col-xl-3">
                        <div class="position-relative">
                            <img src="{{ Storage::url($gallery) }}" data-image-src="{{ $gallery }}" class="mw-100 pic-item rounded shadow preview-element-image" alt="{{ $product->name }}">
          ...
          <form action="{{ route('test_update', $product) }}" method="POST" enctype="multipart/form-data" class="edit-product {{ $errors->all() == true ? 'was-validated' : '' }}" novalidate id="add_product" onsubmit="return false">
            ...
            <div>
                ...
                <div class="form-group d-flex flex-row mb-5 mb-lg-2 justify-content-center justify-content-md-end align-items-start align-items-md-center">
                    <button id="submitAddProduct" type="submit" class="add-product-btn w-75 font-weight-bold btn-danger border-0 mb-5 mt-lg-3 py-2 w-lg-75 rounded-11 add_append" @if(Auth::user()->store && $product->store_id == Auth::user()->store->id) data-store="{{ Auth::user()->store->slug }}"@endif>@if($product->product_status_id == 4 || $product->product_status_id == 5)Обновить @else()Изменить@endif</button>
                </div>
