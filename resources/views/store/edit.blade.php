            ...
            <div class="col mb-3">
              ...
              @if($store->is_active)
                <form action="{{ route('ft-store.toggleUser', $store->id) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    <button type="button" class="rounded-11 btn @if($store->is_active == 0 || $store->is_active == 2)btn-success @else btn-outline-secondary @endif my-1 delete-store w-100 mt-3">
